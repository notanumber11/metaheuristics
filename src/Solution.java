...
public class Solution {
    ...
    public ArrayList<Integer> m_MatrizSolution = new ArrayList(SIZE);
...
/*    public Solution(Distances m_DistanceMatriz, String path){
        this.m_DistanceMatriz = m_DistanceMatriz;
        readTraceFromFilePractica3(path);
        solutions.add(m_MatrizSolution);
    }*/

    public Solution(Distances m_DistanceMatriz, String path){
     //   System.out.println("Ejecuando con lectura de random");
        this.m_DistanceMatriz = m_DistanceMatriz;
        generateVorazSolution();
        //readTraceFromFilePractica3(path);
...
/*        for(int i = 0 ; i < randomNumbers.size();i++){
    ...
    public boolean readTraceFromFilePractica1(String path) {
        solutionReadByFile = true;
        m_MatrizSolution.clear();
        Scanner input = null;
        try {
        ...
        while (input.hasNextLine() ) {
            if(linea<=SIZE){
                ...
                double value = Double.parseDouble(input.nextLine());
                int randomInt = (int) (floor(value * SIZE));
                if(linea%2==1){
                 //   System.out.println(linea+ ": Intercambio : " + randomInt + "  , " +previousValue );
                }
                m_Trace.add(randomInt);
